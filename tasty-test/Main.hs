      ...
      [ testGroup "Parsers"
          $ let testParserOnAllInputs parserName parser inputs =
             ...
             in [ testParserOnAllInputs
                  ...
                  -- Alternatives sharing a "(" prefix used to make each level of
                  -- nesting multiply the work, so this took longer than the age
                  -- of the universe at depth 20. The timeout turns a regression
                  -- into a failure rather than a hang.
                  localOption (mkTimeout 10000000)
                    $ testCase "Deeply nested parens"
                    $ let input = Text.replicate 20 "(" <> "a + b" <> Text.replicate 20 ")"
                       in case Parsing.run Parsing.aExpr input of
                            Left err -> assertFailure err
                            Right _ -> return ()
                ],
